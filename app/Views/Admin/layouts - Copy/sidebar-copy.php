<?php
// ==========================================
// 1. DATA & PERMISSIONS (Tetap pakai data asli lu)
// ==========================================
$db = new Database;
$db->query("SELECT setting_key, setting_value FROM app_settings WHERE setting_key IN ('app_name', 'app_tagline')");
...
function isActive($url, $currentUri) { return ($url === $currentUri || (strpos($currentUri, $url) === 0 && $url !== '/admin/dashboard')); }
        ...
        <?php foreach ($adminMenu as $m): ?>
            ...
            ?>
                <div x-data="{ open: <?= $isOpen ? 'true' : 'false' ?> }">
                    ...
                    <div x-show="!sidebarOpen" class="relative group-hover:block">
                        </div>
                </div>
            <?php endif; ?>
        <?php endforeach; ?>
